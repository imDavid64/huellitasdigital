...
-- ==========================================
-- NOMBRE: HUELLITAS_PRODUCTO_GENERAR_SKU_TRG
-- DESCRIPCIÓN: Trigger para asignar automáticamente un código al registrar producto
-- ==========================================
DROP TRIGGER IF EXISTS HUELLITAS_PRODUCTO_GENERAR_SKU_TRG;
DELIMITER //
CREATE TRIGGER HUELLITAS_PRODUCTO_GENERAR_SKU_TRG
BEFORE INSERT ON huellitas_productos_tb
FOR EACH ROW
BEGIN
    IF NEW.PRODUCTO_SKU IS NULL OR NEW.PRODUCTO_SKU = '' THEN
        SET NEW.PRODUCTO_SKU = GENERAR_CODIGO_PRODUCTO_FN();
    END IF;
END //
DELIMITER ;
